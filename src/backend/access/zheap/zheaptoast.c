 ...
 *
 * IDENTIFICATION
 *	  src/backend/access/heap/zheaptoast.c
 *
 *
 * INTERFACE ROUTINES
 ...
 */
ZHeapTuple
zheap_toast_insert_or_update(Relation rel, ZHeapTuple newtup,
							 ZHeapTuple oldtup, int options)
{
	...
	zheap_deform_tuple(newtup, tupleDesc, toast_values, toast_isnull,
					   numAttrs);
	if (oldtup != NULL)
		zheap_deform_tuple(oldtup, tupleDesc, toast_oldvalues,
						   toast_oldisnull, numAttrs);
...
/*
 * Fetch a TOAST slice from a zheap table.
 *
 ...
 *
 * XXX This is identical with heap_fetch_toast_slice() except for the value of
 * the TOAST_MAX_CHUNK_SIZE constant. Can the code be shared somehow?
 */
void
zheap_fetch_toast_slice(Relation toastrel, Oid valueid, int32 attrsize,
