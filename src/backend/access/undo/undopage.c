					 ...
					 UndoRecPtr chunk_start)
{
	 ...
	 * already written the whole chunk header, just account for its length in
	 * deciding from where to start writing the type header.
	 */
	 ...
	 * contain any chunks yet, then make the "first chunk" pointer point to
	 * this.
	 */
	...
	/*
	 * If we're continuing this header from the previous page, then we've got
	 * to indicate the chunk type and where it starts.
	 */
	if (header_offset > 0)
	{
					 ...
					 UndoRecPtr chunk_start, UndoRecordSetType chunk_type)
{
	 ...
	 * contain any records yet, then make the "first record" pointer point to
	 * this.
	 */
	...
	/*
	 * If we're continuing this record set from the previous page, then we've
